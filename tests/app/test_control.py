...
async def test_status_and_pnl_commands() -> None:
    ...
    assert status is not None and "mode=paper" in status.text
    assert pnl is not None and "+1.25" in pnl.text
...
async def test_positions_and_recent_commands() -> None:
    ...
    assert positions is not None and "#7" in positions.text
    assert recent is not None and "pnl=+0.0786" in recent.text
...
async def test_callback_uses_same_read_only_path() -> None:
    ...
    assert reply is not None and "mode=paper" in reply.text
...
async def test_bot_username_suffix_and_unknown_command() -> None:
    ...
    assert status is not None and "mode=paper" in status.text
    assert unknown is not None and "Unknown" in unknown.text
