...
| G2 paper | 🟦 | Core dibangun, ratusan ronde dan zero mismatch belum dibuktikan |
| G3 live | ⛔ | Dilarang sebelum G2 + approval |
...
| 2.4 | Reconciliation + alert | ✅ | ✅ | 2026-07-13 | 61/61 gabungan + Ruff/Black/mypy + 0/0 |
| T.2 | Read-only commands/buttons | 🟦 | ⬜ | | Implementasi dibuat; tunggu VPS |
| T.3 | pause/resume/kill | ⬜ | ⬜ | | Setelah T.2 |

### Scope selesai

- **2.1:** hard limits, rate limit, pause/resume, kill, breakers, mismatch fatal.
...
- **T.1:** non-blocking queue, retry, failure isolation, critical priority, token tidak di event/log.
- **2.4:** order→fill→position→Gamma settlement→PnL→balance; mismatch freeze + critical alert.
- **T.2 pending verification:** whitelist, `/status /balance /pnl /positions /recent /config`, safe inline buttons, secret allowlist.
...
|---|:--:|
| T.1 notifier | ✅ |
| T.2 read-only control | 🟦 |
| T.3 dangerous controls + two-step confirm | ⬜ |
...
T.1 Notifier         ✅
2.4 Reconciliation  ✅
T.2 Read-only        🟦 VPS verification
T.3 Controls         ⬜
G2                    🟦 belum lulus
G3 live               ⛔
```
